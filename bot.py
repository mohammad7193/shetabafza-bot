...
def send_to_telegram(caption, image_topic):
    print(f"در حال جستجوی تصویر زنده برای موضوع: {image_topic}")
    
    headers = {"Authorization": PEXELS_API_KEY}
    # دریافت 15 عکس اول برای جلوگیری از تکراری شدن تصاویر
    ...
    try:
        ...
        if "photos" in pexels_response and len(pexels_response["photos"]) > 0:
            # انتخاب کاملاً تصادفیِ یک عکس از بین 15 عکس یافت شده
            selected_photo = random.choice(pexels_response["photos"])
